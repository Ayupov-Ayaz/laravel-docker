{!! Form::open(['route' => $route, 'id' => 'packsForm']) !!}
...
@foreach(config('translatable.locales') as $locale)
    <div class="form-group row {{ $errors->has($locale.'[name]') ? 'has-error' : '' }}">
        ...
        <div class="col-sm-10">
            {{ Form::text($locale.'[name]', ($pack ? $pack->translateOrDefault($locale)->name : null), ['class' => $errors->has($locale.'.name') ? 'form-control is-invalid' : 'form-control']) }}
